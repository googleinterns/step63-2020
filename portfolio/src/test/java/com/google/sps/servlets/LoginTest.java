...
public final class LoginTest {
  ...
  public void testDoGet() throws ServletException, IOException{
    ...
    if(!mockUser.isUserLoggedIn()){
      when(mockUser.createLoginURL("/nickname.html")).thenReturn("/nickname.html");
      String loginUrl = mockUser.createLoginURL("/nickname.html");
    ...
    String name = "example";
    when(mockUser.createLogoutURL("/")).thenReturn("/");
    String logoutUrl = mockUser.createLogoutURL("/");
    
    mockObj2.addProperty("url", logoutUrl);
  ...
  public void testGetName() throws ServletException, IOException{
    ...
    // PreparedQuery mockPrep = new PreparedQuery();
    // when(mockDatastore.prepare(mockQuery)).thenReturn(mockPrep); //has to return a prepared query
    
    PreparedQuery results = mockDatastore.prepare(mockQuery); //when().thenReturn()
    // when(results.asSingleEntity()).thenReturn(/* an entity */); 
